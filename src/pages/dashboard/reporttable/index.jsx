...
if(reportLoading){
  ...
  return (
    <Box sx={{ display: 'flex', gap: 2, flexDirection: 'column' }}>
      ...
      >
        <Typography variant="subtitle1" sx={{ fontSize: '22px', fontWeight: 600 }}>
        ...
        >
          <Table sx={{ bgcolor: 'white' }} aria-label="spanning table">
              ...
              <TableRow sx={{ bgcolor: '#a7e5fbcc' }}>
                ...
                {ReportTableHeadStaticData?.map((e, i) => (
                  ...
                  <TableCell
                    key={i}
                    align="center"
                    sx={{ fontWeight: 600, fontSize: 'calc(6px + 1vmin)', bgcolor: i === 5 && '#4173F8' }}
                  >
                    {e}
                  </TableCell>
                ))}
            ...
            <TableBody>
              {fetchedData && Object.keys(fetchedData)?.map((key, i) => (
                key !== 'overall' &&
                <TableRow
                  key={i}
                  sx={{
                ...
                >
                  ...
                  {Object.keys(fetchedData[key]).map((innerKey, j) => (
                    ...
                    <TableCell
                      ...
                      sx={{
                        ...
                        bgcolor: innerKey !== 'Your Score' && colorCode(fetchedData[key][innerKey]),
                      }}
                    >
                      { fetchedData[key][innerKey] === 1 ? 'Need Improvement' : fetchedData[key][innerKey] === 2 ? 'Ready' : fetchedData[key][innerKey]}
                    </TableCell>
                  ))}
                </TableRow>
              ))}
      ...
      >
        <Typography variant="subtitle1" sx={{ fontSize: '22px', fontWeight: 600, pb: 2 }}>
          How ready are you to engage in these levels of analytics
        </Typography>
        <Grid container spacing={{ xs: 2, md: 3 }}>
          {fetchedData && Object.keys(fetchedData?.overall)?.map((e, index) => (
            <Grid item xs={12} sm={6} md={4} lg={3} key={index}>
              {index !== ReportTableHeadStaticData.length - 1 && (
                ...
                >
                  ...
                  <Chip sx={{ bgcolor: '#DACAD7', fontSize: '15px' }} label={fetchedData?.overall[e]} />
                </Stack>
              )}
            </Grid>
          ))}
